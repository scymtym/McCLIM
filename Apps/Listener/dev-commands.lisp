...
(defun display-evalues (values)
  (labels
      ((present-value (value)
         ...
         (let ((state (make-instance 'new-inspector::inspector-state
                                     :root-object value)))
           (updating-output (*standard-output* :all-new t)
             (new-inspector::present-inspected-object-graph state *standard-output*)))
         #+no (with-output-as-presentation (t value (presentation-type-of value)
                                              :single-box t)
