...
(defun request-frame-data (port connection sheet)
  (when-let ((mirror (climi::port-lookup-mirror port sheet))) ; TODO hack
    (when (old-back-buffer mirror)
      (let ((event (make-instance 'climi::swap-buffers-event :sheet sheet :timestamp 0)))
        (dispatch-event sheet event)

        (climi::wait-for-state event 1)
        ...
        (clim-internals::enter-state event 2))

      #+old (flet ((upload-frame-data (pixels dirty-region)
