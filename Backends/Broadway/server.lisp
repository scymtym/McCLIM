...
(defun run-server ()
  (usocket:with-socket-listener (server-socket "0.0.0.0" 8084)
...
(defvar *client* (alexandria:read-file-into-string "client.html"))
...
(defvar *js* (alexandria:read-file-into-string "broadway.js"))
...
(defun serve-js (socket)
  (let ((stream (usocket:socket-stream socket)))
    ...
    (write-crlf-line "" stream)
    (write-string (alexandria:read-file-into-string "broadway.js") stream)))
  ...
  (let ((stream (usocket:socket-stream socket))
    ...
    (upload-texture
     ...
     (let* ((port   (make-instance 'port2))
       ...
       (draw-text* pixmap "Broadway" 20 40 :ink +green+)
       (draw-text* pixmap "Test" 20 60 :ink +blue+)
          ...
          :do
             ...
             (upload-texture
              ...
              (let* ((port   (make-instance 'port2))
                ...
                (draw-text* pixmap "Test" 20 60 :ink (make-contrasting-inks 8 (mod (floor i 4) 8)))
             ...
             (set-nodes stream 1 (list (make-instance 'texture
                                                      :x (+ 50.0f0 (* 20 (sin (/ i 10)))) :y 20.0f0 :width 100.0f0 :height 100.0f0
                                                      :id 5))
                        :delete t)
                         ...
                         (let* ((mask?   (logbitp 7 (aref buffer 1)))
                           ...
                           (when (plusp length)
                             (loop :for offset = 0 :then (read-sequence payload stream :start offset)
                                   :while (< offset length))
                             (handle-incoming-message (if mask? (unmask payload) payload)))))))))
