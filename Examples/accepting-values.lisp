...
(defun accepting-interval (&key (min -1.0) (max 1.0) (stream *query-io*) (ow t))
  (accepting-values (stream :resynchronize-every-pass t :own-window ow)
    (fresh-line stream)
    (setq min (accept 'real :default min :prompt "Min":stream stream))
    (fresh-line stream)
    (setq max (accept 'real :default max :prompt "Max" :stream stream))
    (when (< max min)
      (rotatef min max)))
  (values min max))
    ...
    (restart-case
        (progn
          (accepting-values (stream :own-window ow)
            ...
            (setq month (accept 'integer :stream stream
                                         :default month :prompt "Month"))
            (terpri stream)
            (setq day (accept 'integer :stream stream
                                       :default day :prompt "Day"))
            (terpri stream)
            (setq hour (accept 'integer :stream stream
                                        :default hour :prompt "Hour"))
            (terpri stream)
            (setq minute (accept 'integer :stream stream
        ...
        (progn
          (accepting-values (stream :own-window ow)
            ...
            (setq minute (accept 'integer :stream stream
                                          :default minute :prompt "Minute"))
            (terpri stream)
            (accept-values-command-button (stream) "Zero seconds"
              (setq second 0)))
  ...
  (menu-choose '(circle square triangle)
               :printer #'(lambda (item stream)
                            (with-drawing-options (stream :ink +yellow+)
                              (case item
                                (circle (draw-circle* stream 0 0 10))
                                (square (draw-polygon*
