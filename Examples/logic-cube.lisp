...
;;; ---------------------------------------------------------------------------
;;;
;;;  (c) Copyright 2006 Andy Hefner <ahefner@gmail.com>
;;;
;;; ---------------------------------------------------------------------------
;;;
...
(defclass logic-cube-pane (clime:never-repaint-background-mixin basic-gadget)
  ((%background :initform (make-rgb-color 0.35 0.35 0.46)
...
(defun reset-logic-cube (cube new-density)
  (with-slots (density playfield squeeze drag-color dragging flyaway decorator) cube
    (setf density new-density
          ...
          flyaway 0.0
          squeeze nil
          playfield (make-array (list 3 density density)
...
(defparameter *logic-cube-colors*
...
(defun generate-better-cube-puzzle (cube &optional (num-colors 6))
  ;; The puzzles coming out of the above were so bad that I threw this
  ;; together to reject some of the obviously awful ones.
  (loop for i from 0 below 100
        do (generate-cube-puzzle cube num-colors)
...
(defmethod compose-space ((pane logic-cube-pane) &key width height)
  ...
  (make-space-requirement :min-width 200 :width 550
                          :min-height 200 :height 550))
...
(defun won-logic-cube (pane)
  (let ((start-time (get-internal-real-time))
    ...
    (loop with sequence-length = 2.0
          for frame-start-time = (get-internal-real-time)
          for progress = (/ (- frame-start-time start-time)
                            internal-time-units-per-second sequence-length)
          while (< progress 1.0)
          do (setf (squeeze pane) (1- progress)
                   (flyaway pane) (expt progress 0.55)
                   (yaw pane)
                   (let ((foo (if (< progress spin-start-progress)
                                  0.0
                                  (/ (- progress spin-start-progress)
                                     (- 1.0 spin-start-progress)))))
             ...
             (medium-finish-output (sheet-medium pane))
             (let* ((frame-end-time (get-internal-real-time))
                    (remainder (- 0.01 (- frame-end-time frame-start-time))))
               (when (plusp remainder)
                 (sleep (/ remainder internal-time-units-per-second))))))
