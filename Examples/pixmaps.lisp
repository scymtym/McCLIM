...
(defun repaint-sheet-if-pixmap-sheet (sheet)
...
(defvar *source-panes* '(app1 app2 pix1 pix2))

(defvar *destination-panes* (list* 'half-dead *source-panes*))

(defvar *repaint-panes* (list* 'dead *destination-panes*))

(defun source-tester (object &key &allow-other-keys)
  (member (pane-name (event-sheet object)) *source-panes*))
...
(define-drag-and-drop-translator tr-select-region
  ...
  (let ((source-sheet (event-sheet object))
    ...
    (region-feedback nil nil source-sheet x1 y1 x2 y2 :highlight)
    (let* ((frame *application-frame*)
           (panes (map 'list (alexandria:curry #'find-pane-named frame)
                       *source-panes*))
           (half-dead (find-pane-named frame 'half-dead))
           (dest (with-input-context (`(or ,@(map 'list (lambda (pane)
                                                          `(blank-area :sheet ,pane))
                                                  panes)
      ...
      (region-feedback nil nil source-sheet x1 y1 x2 y2 :unhighlight)
      `(com-copy ,(pane-name source-sheet)
                 ,(pane-name dest-sheet)
                 ,x1 ,y1 ,(- x2 x1) ,(- y2 y1)
                 ,dest-x ,dest-y))))

(define-presentation-action action-repaint-sheet
    (blank-area command pixmaps
                :gesture :describe
                :tester ((object) ; disallow e.g. pointer documentation pane
                         (member (pane-name (event-sheet object))
                                 *repaint-panes*))
                :documentation "Repaint sheet")
    (object)
...
(define-pixmaps-command (com-copy :name "Copy")
    ((arg1 `(member ,@*source-panes*))
     (arg2 `(member ,@*destination-panes*))
     (from-x 'integer)
     (from-y 'integer)
     (width 'integer)
     (height 'integer)
  ...
  (let* ((frame *application-frame*)
    ...
    (when (typep pane1 'pixmap-sheet)
      (repaint-sheet pane1 +everywhere+))
    (when (and (typep pane2 'pixmap-sheet) (not (eq pane1 pane2)))
      (repaint-sheet pane2 +everywhere+))))
