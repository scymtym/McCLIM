...
(defun display (frame pane)
  ...
  (let* ((string "aTΣ音◌᷉")
    ...
    (with-drawing-options (pane :transformation (make-translation-transformation x-offset y-offset)
      ...
      (draw-line* pane 0 0 0 (* 7 y-offset))
      (draw-line* pane 0 0 (* 9 x-offset) 0)
      (draw-line* pane (* 3 x-offset) 0 (* 3 x-offset) (* 7 y-offset))
      (draw-line* pane (* 6 x-offset) 0 (* 6 x-offset) (* 7 y-offset))
      ;; Reference
      (draw-text* pane string 0 0 :transform-glyphs nil :ink +dark-red+)
      ...
      (with-drawing-options (pane :transformation
        ...
        (let ((toward-x (* 8 x-offset))
              (toward-y (* -1 y-offset)))
          (draw-point* pane 0 0 :ink +blue+ :line-thickness 10)
          (draw-point* pane toward-x toward-y :ink +red+ :line-thickness 10)
          (draw-line* pane 0 0 toward-x toward-y :ink +magenta+)
          (let ((string "The quick brown fox jumps over the lazy dog"))
      ...
      (do* ((scalings-x '(+1.5 +0.7 -1.0 -1.5 -0.7 +1.5 -1.5) (cdr scalings-x))
        ...
        (with-drawing-options (pane :transformation (make-translation-transformation
          ...
          (let ((dx (or (car pair) 0))
                (dy (cdr pair)))
            (draw-point* pane sx sy :ink +blue+ :line-thickness 10)
            (draw-point* pane (or dx 0) (or dy 0) :ink +red+ :line-thickness 10)
            (draw-line* pane sx sy (or dx 0) (or dy 0) :ink +magenta+)
            (draw-text* pane string sx sy :transform-glyphs t :ink +grey+)
            (draw-text* pane string sx sy :transform-glyphs t :ink +dark-red+
                                          :toward-x dx
                                          :toward-y dy)))
        ;; Rotations
        (with-drawing-options (pane :transformation
                                    ...
                                    :text-style text-style)
          (draw-point* pane 0 0 :ink +blue+ :line-thickness 10)
          (draw-text* pane string 0 0 :transform-glyphs t :ink +dark-red+)
          (draw-line* pane 0 0 50 0))
        ;; Scaling
        (with-drawing-options (pane :transformation
                                    (compose-transformation-with-translation
        ...
        (with-drawing-options (pane
          ...
          (let ((dx (car pair))
            ...
            (draw-line* pane 0 0 50 0)
            (draw-text* pane string 0 0 :transform-glyphs t :ink +grey+)
            ;; with towards is red
            (draw-line* pane 0 0 (or dx 0) (or dy 0) :ink +magenta+)
            (draw-text* pane string 0 0 :transform-glyphs t :ink +dark-red+
                                        :toward-x dx
                                        :toward-y dy)))))))
